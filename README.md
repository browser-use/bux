...
## Your 24/7 Hermes agent with a real browser, on any box you own.
...
Three minutes from a blank VPS to *"hey Hermes, check my email and summarize the unread ones"* via Telegram.
...
Paste into your local agent or terminal assistant and it will set up your VPS for you:
...
- **Hermes Agent** configured and always on
...
- When Hermes hits a login wall / 2FA / CAPTCHA, it hands you a **live view URL** and waits — no credential-stuffing, no brittle workarounds
...
- A Hermes-supported model provider key or account. Run `hermes setup` after install. Legacy Claude Code and Codex login flows are still available as additional providers.
...
```
  telegram ──►  telegram_bot.py ─┐
                                 ├──► hermes chat ─► browser-harness ──► BU Cloud
...
- [agent/SOUL.md](agent/SOUL.md) — Hermes identity and voice.
- [agent/system-prompt.md](agent/system-prompt.md) — bux project instructions loaded through `HERMES.md` / `AGENTS.md`. Edit it to customize behavior, then rerun `install.sh` — it's idempotent and the next Hermes turn picks up the change.

## Managed offering

If you'd rather not run your own VPS: [cloud.browser-use.com](https://cloud.browser-use.com) provisions a box for you in ~60s — same software, zero setup.
