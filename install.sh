...
# NOTE: we deliberately do NOT grant the bux user broad / unscoped sudo.
# `sudo apt install <arbitrary deb>` runs maintainer scripts as root and is
# therefore root-equivalent — same for dpkg, npm, pipx, snap. Anything we'd
# whitelist beyond the narrow allowlist below would break the boundary that
# keeps the TG bot's bot-token (root-owned at /etc/bux/tg.env) safe from a
# compromised bux user.
#
# The narrow exception — installed by setup_sudoers() further down, after the
# systemd units exist — is a small allowlist of `systemctl restart|status|
# is-active` and `journalctl -u` calls scoped to the bux-* services. That
# lets the agent recover its own services without prompting; it does NOT
# let the agent start arbitrary services, edit unit files, or read other
# units' logs. See setup_sudoers() for the exact allowlist.
#
...
# --- scoped passwordless sudo for the bux agent ---------------------------
# Lets the agent restart / inspect its own services and read their logs
# without a password prompt — nothing more. The allowlist is *narrow on
# purpose*: each entry pins a full path (defeats $PATH attacks) and a
# specific unit name (defeats `systemctl restart anything-else`). Any
# expansion of this list needs careful review — see SECURITY in the PR.
setup_sudoers() {
	local systemctl_bin journalctl_bin tmp
	systemctl_bin="$(command -v systemctl || true)"
	journalctl_bin="$(command -v journalctl || true)"
	[ -n "$systemctl_bin" ] || die 'setup_sudoers: systemctl not found in PATH'
	[ -n "$journalctl_bin" ] || die 'setup_sudoers: journalctl not found in PATH'

	tmp="/tmp/bux.sudoers.$$"
	# Heredoc with substituted absolute paths. Sudoers requires the file be
	# mode 0440 and pass `visudo -c` — anything else is silently ignored
	# (or worse, breaks all sudo on the box). Validate before installing.
	cat > "$tmp" <<EOF
# Allow the bux agent to manage its own services without a password.
# Scoped to specific commands for safety — DO NOT broaden without review.
# Generated by bux install.sh; revoke with: sudo rm /etc/sudoers.d/bux

bux ALL=(root) NOPASSWD: ${systemctl_bin} restart bux-tg
bux ALL=(root) NOPASSWD: ${systemctl_bin} restart bux-tg.service
bux ALL=(root) NOPASSWD: ${systemctl_bin} restart bux-browser-keeper
bux ALL=(root) NOPASSWD: ${systemctl_bin} restart bux-browser-keeper.service
bux ALL=(root) NOPASSWD: ${systemctl_bin} restart bux-ttyd
bux ALL=(root) NOPASSWD: ${systemctl_bin} restart bux-ttyd.service
bux ALL=(root) NOPASSWD: ${systemctl_bin} status bux-tg, ${systemctl_bin} status bux-tg.service
bux ALL=(root) NOPASSWD: ${systemctl_bin} status bux-browser-keeper, ${systemctl_bin} status bux-browser-keeper.service
bux ALL=(root) NOPASSWD: ${systemctl_bin} status bux-ttyd, ${systemctl_bin} status bux-ttyd.service
bux ALL=(root) NOPASSWD: ${systemctl_bin} is-active bux-tg, ${systemctl_bin} is-active bux-browser-keeper, ${systemctl_bin} is-active bux-ttyd
bux ALL=(root) NOPASSWD: ${journalctl_bin} -u bux-tg *
bux ALL=(root) NOPASSWD: ${journalctl_bin} -u bux-browser-keeper *
bux ALL=(root) NOPASSWD: ${journalctl_bin} -u bux-ttyd *
EOF

	if ! visudo -c -f "$tmp" >/dev/null; then
		rm -f "$tmp"
		die 'setup_sudoers: generated sudoers file failed visudo validation'
	fi
	install -m 0440 -o root -g root "$tmp" /etc/sudoers.d/bux
	rm -f "$tmp"
	ok 'scoped sudo granted to bux for systemctl/journalctl on bux-* services'
}
