...
class Bot:
	...
	def handle(self, msg: dict) -> None:
		...
		# as if the user had typed the transcript. Media can never be a
		# `/start <token>` binding message (those are text-only), so an
		# unbound chat that sends audio is silently dropped here without
		# burning an OpenAI call.
		file_id, filename, file_size = _extract_media(msg)
		if file_id and not text:
			if chat_id not in allow:
				LOG.info('dropping media msg from chat_id=%s (not bound)', chat_id)
				return
		...
		# and sends the result reply when it's done.
		job = {
			'id': _new_job_id(),
			'chat_id': chat_id,
			'message_id': mid,
