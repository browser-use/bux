...
def _kill_inflight_proc(slug: str, proc: subprocess.Popen, reason: str) -> None:
    ...
    delays the runner's final cancellation handling. Popen uses
    start_new_session=True for agent turns, so kill the whole process group.
    """
...
class Bot:
    ...
    def handle(self, msg: dict) -> None:
        ...
        # message with no "got it" signal. React on enqueue so the
        # ack is tied to Telegram ingestion, not agent dispatch timing.
        if isinstance(mid, int):
