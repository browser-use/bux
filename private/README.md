...
- **Personal skills** — domain-specific `SKILL.md` files for your own bux that you don't want shared upstream. Either drop them directly here, or symlink from `~/.hermes/skills/<name>/` into `private/skills/<name>/`.
...
bux stores agent-managed memory outside the repo, primarily under `/home/bux/.hermes/`. Legacy Claude installs may also have `/home/bux/.claude/projects/-opt-bux-repo/memory/`. Keep private, user-specific memory out of this public repo.
